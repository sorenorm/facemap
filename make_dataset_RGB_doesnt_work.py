...
data = torch.zeros((len(img_files), 3, h, w))
...
for i in range(len(img_files)):
    ...
    im = plt.imread(
        img_files[i]
    )  # Load the image in RGB format without channel reduction

    # Crop and resize as before
    ...
    data[i] = torch.Tensor(im_r / 255.0).permute(
        2, 0, 1
    )  # Rearrange to (channels, height, width) for RGB
